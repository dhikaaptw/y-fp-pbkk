        ...
        <div class="profile-card">
          <div class="profile-row">
            ...
            <div>
              ...
              <div class="profile-bio">Minimal microblog profile.</div>
...
def render_profile_posts(uname):
    st.markdown('<div class="section-title">Posts</div>', unsafe_allow_html=True)
    res = api.api_search_posts(uname, st.session_state.token)
    if res.status_code == 200:
        user_posts = [p for p in res.json() if p["owner"]["username"] == uname]
        if not user_posts:
            st.markdown('<div class="empty-state">No posts yet</div>', unsafe_allow_html=True)
        else:
            for index, post in enumerate(user_posts):
                render_post_card(post, show_actions=True, ctx=f"profile_{index}_")
    else:
